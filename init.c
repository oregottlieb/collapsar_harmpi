...
void init_collapsar()
{
  ...
  rbr = 10*Rout/1e5; // beginning of the lower res. patch
  npow2=4.0; // power exponent
  cpow2=1.0; // exponent prefactor (the larger it is, the more hyperexponentiation is)
  ...
  ZSLOOP(0,N1-1,0,N2-1,0,N3-1) {
    ...
    if(r < Rs) {
      ...
      up = 0; //mega0/(1+pow(r/A_rot,2))/3e10; // angular velocity
    }
    else {
      rho = 1.e-7*RHOMIN ;
      u = 1.e-7*UUMIN ;
    ...
    p[i][j][k][U2] = uh ;
    p[i][j][k][U3] = up ;
    coord_transform(p[i][j][k],i,j,k) ;  // convert from 4-vel to 3-vel
 ...
 /* ZSLOOP(0,N1-1+D1,0,N2-1+D2,0,N3-1+D3) {
     coord(i,j,k,CENT,X) ;
     bl_coord(X,&r,&th,&phi) ;
     A[i][j][k] = Ap*pow(sin(th),2)*r*r/(pow(r,3)+pow(Fe_core,3)); // dipole magnetic field
    }
 */
  fixup(p) ;
  //bsq_max = compute_B_from_A(A,p);
